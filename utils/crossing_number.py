...
def calculate_minutiaes(im, kernel_size=3):
    binary_image = im.astype(np.int8)
    endings = []
    bifurcations = []

    (y, x) = im.shape
    result = cv.cvtColor(im*255, cv.COLOR_GRAY2RGB)
    ...
    for i in range(1, x - kernel_size//2):
        for j in range(1, y - kernel_size//2):
            ...
            if minutiae != "none":
                cv.circle(result, (i,j), radius=1, color=colors[minutiae], thickness=2)
                if (minutiae == "ending"):
                    endings.append((i, j))
                elif (minutiae == "bifurcation"):
                    bifurcations.append((i,j))

    return result, endings, bifurcations